...
| `/read <url>` | Starts tracking a book. Scrapes Goodreads, creates a thread in the member's forum channel (with "Bot" tag), upserts the `Book` record, opens a `ReadingLog`, and posts an initial 0% progress bar to `#progress` for personal books. |
| `/progress [page] [percentage]` | Logs reading progress from inside a bot-managed thread. Exactly one of `page` or `percentage` required. Posts a compact progress bar in the thread. For personal books also posts/edits a bar in `#progress`. At 100% automatically marks the book finished (posts completion embed, links to epilogue if club read). Resuming an abandoned book removes the "Abandoned" forum tag. |
...
| `/abandon` | Marks the book as abandoned at current progress. Posts an embed in the thread showing progress at abandonment. For personal books also posts/edits a bar with `✗` in `#progress`. |
...
| `ReadingLog` | One entry per member per book thread. `threadId` is unique and routes `/progress`, `/rate`, `/abandon`. Fields: `progress` (real 0–100), `rating` (real? 1–5), `status` (`"reading"` \| `"finished"` \| `"abandoned"` \| `"dnr"`), `startedAt`, `finishedAt`, `progressMessageId` (Discord message ID of the personal-book `#progress` entry, edited in-place on updates). Multiple logs per user+book are allowed (re-reads). `"dnr"` (did not read) is set by import scripts for historical BOTM books a member was enrolled in but never started. |
...
- `/read` scrapes the book, fetches the member's forum channel (gives a user-friendly error if the channel is gone), applies the "Bot" tag if available, creates the thread (name truncated to 100 chars), upserts `Book`, creates `ReadingLog`, calls `updateProgressPost`, then for personal books posts an initial 0% bar to `#progress` and saves the message ID as `progressMessageId` on the log.
- `/progress` validates Bot tag + ownership + not-finished. Converts page → percentage using `book.pages`. If result ≥ 100: sets `status = "finished"`, `finishedAt = now()`, `progress = 100`, posts completion embed, links to epilogue if club read. Otherwise updates `progress` (and resets `status = "reading"` if previously abandoned, removing the "Abandoned" forum tag). Posts a compact bar in the thread. For personal books, also posts/edits a bar in `#progress` (edit-in-place via `progressMessageId`; falls back to a new message if deleted). Calls `updateProgressPost`.
...
- `/abandon` validates Bot tag + ownership + not-finished + not-already-abandoned. Sets `status = "abandoned"`. Posts embed showing progress at time of abandonment. For personal books, posts/edits a bar with `✗` in `#progress` (same edit-in-place pattern). Calls `updateProgressPost`.
